Shader "Custom/water_rgb" {
	Properties {
		...
		_TintColor_r ("Tintcolor_r", color) = (1,1,1,1)
		...
		_Scrolloffset_b ("b_scrolloffset",float) = 0.5
...
return c;
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			half4 d = tex2D (_MainTex,  float2(IN.uv_MainTex.x*_ScrollXtile_g-_Time.x*_ScrollX_g,IN.uv_MainTex.y*_ScrollXtile_g-_Time.y*-_ScrollY_g));
			half4 f = tex2D (_MainTex, IN.uv_MainTex);
			o.Emission = ((c.r * _TintColor_r) + (d.g * _TintColor_g) + (f.b * _TintColor));
			o.Alpha = (((c.r * _Scrolloffset_r * _TintColor_r) + (d.g  * _Scrolloffset_g * _TintColor_r)) + (f.b))* f.b +_BaseAlphaScroll;
		}
		ENDCG
	} 
	FallBack "Diffuse"
